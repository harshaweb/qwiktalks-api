...
async function trialPeriodCronService() {
    cron.schedule('0 1 * * *', async () => {
        console.log('Trial expired user deletion cron job skipped - data deletion is permanently disabled.');
        return;

        // NOTE: The destructive deletion logic below is disabled to prevent data loss.
        // To re-enable, remove the `return;` above and ensure trial_expired_delete_days is properly configured.
        /*
        try {
    ...
    console.log('Trial expired user deletion cron job scheduled. (Deletion is disabled)');
}
