...
export const processCampaignMessageJob = async (jobData) => {
  ...
  try {
    ...
    let result;
    const wabaDoc = selectedPhoneNumber.waba_id;
      ...
      const aisensyPayload = {
        ...
        template: {
          ...
          language: {
            ...
            code: templateData.language_code || 'en'
          },
          components: templateComponents
        }
      };
      const aisensyResult = await aisensyService.sendMessage(aisensyPayload);
      result = {
        id: aisensyResult.data?.messages?.[0]?.id || aisensyResult.messages?.[0]?.id || 'unknown',
        provider: 'aisensy'
      };
    } else {
      try {
        result = await unifiedWhatsAppService.sendMessage(userId, messageParams);
      } catch (businessError) {
        const errorMessage = businessError.message || '';
        const isPermissionError = errorMessage.includes('(#10)') ||
          errorMessage.includes('necessary permissions') ||
