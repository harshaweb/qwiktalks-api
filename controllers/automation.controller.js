...
export const getAutomationFlows = async (req, res) => {
  try {
    ...
    let flows = await AutomationFlow.find(searchQuery)
      .sort({ [sortField]: sortOrder })
      .skip(skip)
      .limit(parseInt(limit))
      .populate('user_id', 'name email');

    let total = await AutomationFlow.countDocuments(searchQuery);

    // If no automation flows in DB, sync from AiSensy WhatsApp flows
    if (total === 0 && !req.query.search) {
      try {
        console.log('[getAutomationFlows] No flows in DB, trying AiSensy sync');
        ...
        for (const item of items) {
          const existing = await AutomationFlow.findOne({ name: item.name, user_id: userId });
          if (!existing) {
            await AutomationFlow.create({
              user_id: userId,
              name: item.name,
              description: `AiSensy WhatsApp Flow (${item.categories?.join(', ') || 'Unknown'})`,
        ...
        flows = await AutomationFlow.find(searchQuery)
          .sort({ [sortField]: sortOrder })
          .skip(skip)
          .limit(parseInt(limit))
          .populate('user_id', 'name email');
        total = await AutomationFlow.countDocuments(searchQuery);
        console.log('[getAutomationFlows] Synced', total, 'flows from AiSensy');
      } catch (syncError) {
        console.error('[getAutomationFlows] AiSensy sync failed:', syncError.message);
      }
    }
